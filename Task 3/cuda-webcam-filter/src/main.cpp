...
int main(int argc, char **argv)
{
    ...
    {
        PLOG_INFO << "HDR Parameters - Exposure: " << options.exposure
                  << ", Gamma: " << options.gamma
                  << ", Saturation: " << options.saturation
                  << ", Algorithm: " << static_cast<int>(options.toneMappingAlgorithm);
    }
    ...
    while (true)
    {
        ...
        if (filterType == cuda_filter::FilterType::HDR_TONEMAPPING)
        {
            int algorithmIndex = static_cast<int>(options.toneMappingAlgorithm);
            cuda_filter::applyHDRToneMappingCPU(frame, filteredCPU, 
                                               options.exposure, options.gamma, options.saturation, algorithmIndex);
        }
        else
        {
        ...
        {
            int algorithmIndex = static_cast<int>(options.toneMappingAlgorithm);
            cuda_filter::applyHDRToneMappingGPU(frame, filteredGPU, 
                                               options.exposure, options.gamma, options.saturation, algorithmIndex);
        }
        else
        {
