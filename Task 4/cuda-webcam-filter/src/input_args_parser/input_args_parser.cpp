...
namespace cuda_filter
{
    ...
    FilterOptions InputArgsParser::parseArgs()
    {
        ...
        // Pipeline parameters
        filterOptions.pipelineFilters = result["pipeline"].as<std::string>();
        filterOptions.enablePipeline = !filterOptions.pipelineFilters.empty();
        filterOptions.enableMultiStream = result.count("multi-stream") > 0;
        filterOptions.enableTransitions = result.count("transitions") > 0;
        filterOptions.transitionDuration = result["transition-duration"].as<float>();
        filterOptions.showPerformanceMetrics = result.count("show-metrics") > 0;
    ...
    void InputArgsParser::setupOptions(cxxopts::Options &options)
    {
        options.add_options()
            ...
            ("pipeline", "Pipeline filters (comma-separated): blur,sharpen,hdr", cxxopts::value<std::string>()->default_value(""))
            ("multi-stream", "Enable multi-stream processing")
            ("transitions", "Enable filter transitions")
            ("transition-duration", "Transition duration in seconds", cxxopts::value<float>()->default_value("2.0"))
            ("show-metrics", "Show detailed performance metrics")
